...
function update1() {
    console.log("分数更新...")
    var rows = $('tbody.reactable-data').children();
    // var rows = $('tbody.reactable-data > tr');
    if (rows == undefined) {
        console.log("Failed to get problems, 请稍等...")
        return;
    }
    else{
...
function addObserverIfDesiredNodeAvailable(observer) {
    var problem = $("tbody.reactable-data > tr")[1]
    if(!problem) {
        //The node we need does not exist yet.
        //Wait 500ms and try again
        window.setTimeout(addObserverIfDesiredNodeAvailable(observer), 500);
        return;
    }
    var config = {
        attributes: true,
    ...
    observer.observe(problem, config);
}
...
async function init() {
    ...
    }else if (url.includes("tag/dynamic-programming")){
        // update1();  // 更新
        var observer = new MutationObserver(debounce(update1, 1000)); // 延迟半秒更新
        // 监听第一行
        // observer.observe($("tbody.reactable-data > tr")[1], config); 
        // delay observe until the problem table is ready  
        var problem = $("tbody.reactable-data > tr")[1];        
        while (!problem) {
            //The node we need does not exist yet.
            //Wait 500ms and try again
            ...
            await sleep(500);
            problem = $("tbody.reactable-data > tr")[1];
        }
        update1();  // 更新
        observer.observe(problem, config);
        // 监听头
